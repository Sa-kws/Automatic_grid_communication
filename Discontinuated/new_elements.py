...
class Grid():
    PAGES = []
    SLOTS = []
    LISTE_CORE = [['je', 0, 1], ['vouloir', 0, 0], ['aimer', 1, 0], ['quoi', 1, 1], ['pourquoi', 2, 3], ['Ici', 1, 3]]
...
class Page():
    ...
    def createPage(self, core_voc):
        ...
        for vocab in core_voc:
            row = vocab[1]
            col = vocab[2]
            try:
                tableau[row][col] = vocab
            except IndexError:
    ...
    def addSlots(self, liste_slots, tableau):
        # liste_slot = une liste de slots de type : [row, column, word]
        # tableau = une liste de liste de liste : [  [[None][None]...] [[None][core, row, column]...] ...  ]
        #   --> Il s'agit de la page que nous avons créée via Page().createPage()
        for row in tableau:
            for column in range(0,len(row)):
                if row[column][0] == None:
                    for slot in liste_slots:
                        row_slt = slot[0]
                        col_slt = slot[1]
                        try:
                            tableau[row_slt][col_slt] = slot
                        except IndexError:
                            print('SLOTS NON AJOUTES :\nErreur d\'index - La position du vocabulaire core n\'existe pas dans la grille.\nChangez la taille de la grille, ou la position du vocabulaire core.')
                            return 'SLOTS NON AJOUTES :\nErreur d\'index - La position du vocabulaire core n\'existe pas dans la grille.\nChangez la taille de la grille, ou la position du vocabulaire core.'
            print('\tLes slots sont ajoutés à la page.')
        return tableau
...
'''
    def old_addSlots(self, slots, page):
...
class Slot():
    ...
    def addItem(self, item_list, page):
        ...
        # item must be list
        position = []
        print('Longeur de la liste d\'ìtem :', len(item_list))
        print('Nombre de place disponibles :', (Page.ROW_SIZE * Page.COL_SIZE) - len(Grid.LISTE_CORE))
        if len(item_list) == ((Page.ROW_SIZE) * (Page.COL_SIZE)) - len(Grid.LISTE_CORE):
            pos = 0
            row = 0
            while row < Grid.ROW_SIZE and page[row][col] == None:
                col = 0
                while col < Grid.COL_SIZE:
                    ...
                    slot.append(row)
                    slot.append(col)
                    slot.append(item_list[pos])
                    self.LISTE_SLOTS.append(slot)
                    pos += 1
                row += 1
...
class Item():
    ...
    def __init__(self, word):
        self.Item = Item
        self.WORD = word
        # ceci implique que le word doit être passé en argument lorsque l'on créer la classe
